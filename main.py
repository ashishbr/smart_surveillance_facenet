...
def check_ot():
    otp=int(Entry1.get())
    res_lbl=Button(win,text="",bg="black",fg="white",font=("times",25,"bold"))
    res_lbl.place(x=150,y=420)
    if otp_org==otp:
        res="successfully \n authenticated"
        res_lbl.configure(text=res,height=2,fg="green",width=20,command=sucess)
    else:
        res="otp not mached"
        res_lbl.configure(text=res,height=2,fg="red",width=20,command=exit_win)
        
    
        
    
def check_otp(matching_id):
    global otp_org,win
    otp_org=int(random.randint(1000, 9999))
    print(otp_org)
    win=Tk()
    win.title("cheking otp")
    win.configure(bg="black")
    win.geometry("500x500")
    label=Label(win,text=" Enter OTP",bg="black",fg="white",font=("times",25,"bold"))
    label.place(x=100,y=25)
    global Entry1
    Entry1=Entry(win,text="",bg="black",fg="white",font=("times",25,"bold"))
    Entry1.place(x=100,y=150)

    label=Button(win,text="Submit",bg="black",fg="white",font=("times",25,"bold"),command=check_ot)
    label.place(x=150,y=340)
    win.mainloop()
                ...
                if len(face_patches) > 0:
                    ...
                    for bb, landmark, matching_id, dist in zip(
                        ...
                        if matching_id is None:
                            ...
                            bot = telepot.Bot("7144438130:AAFKXV2JX3fuHF3bQiQucj6LSTMoEskGI38")
                            bot.sendMessage("6219725990", str("UNKNOWN PERSON DETECTED"))
                            bot.sendPhoto("6219725990", photo=open("frame.png","rb"))                           
                            count=0
                        else:
